...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const playTest = () => {
        const finalAnswers = [];
        const obj = {};//инфа о заказе(что навыбирал покупатель) или заказ к исполнению
        ...
        const renderQuestions = (indexQuestion) => {//отрисовка вопросов и ответов
            ...
            if (numberQuestion === 0) {//в первом экране убрать кнопку ПРЕВ
                prevButton.classList.add('d-none');
            }

            if (numberQuestion === questions.length) {//экран с вводом телефона
                questionTitle.textContent = '';//убрать заголовок (следующего нет, висит предыдущий про Соус)
                ...
                const numberPhone = document.getElementById("numberPhone");//поле ввода номера телефона
                numberPhone.addEventListener('input', (event) => {
                    // console.log(event.target.value);
                    // event.target.value = event.target.value.replace(/\D/,"");//замена нецифрового ввода(буквам запрет на ввод)
                    event.target.value = event.target.value.replace(/[^0-9+-]/,"");
                });
            }
            if (numberQuestion === questions.length + 1) {//финальный экран
                formAnswers.textContent = 'Thank you for quiz!';
                sendButton.classList.add('d-none');//убрать кнопку Сенд
